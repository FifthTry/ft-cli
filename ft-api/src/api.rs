...
fn get_util(url: &str) -> crate::FTResult<serde_json::Value> {
        ...
        .send()
    {
        ...
        Err(e) => {
            Err(crate::error::Error::APIError{error: e}.into())
        }
    }
}

pub fn get<T: serde::de::DeserializeOwned>(url: &str) -> crate::FTResult<ApiResponse<T>> {
    match get_util(url) {
...
fn post_util<B: Into<reqwest::blocking::Body>>(
    url: &str,
    body: B
) -> crate::FTResult<serde_json::Value> {
    let client = reqwest::blocking::Client::new();
    match client
        .post(url)
        .body(body)
        ...
        .send() {
        Ok(response) => if response.status() != reqwest::StatusCode::OK {
            Err(crate::error::Error::APIResponseNotOk("post api response not OK".to_string()).into())
        } else {
            response.json().map_err(|e| e.into())
        },
        Err(e) => {
...
pub fn post<T: serde::de::DeserializeOwned, B: Into<reqwest::blocking::Body>>(url: &str, body: B) -> crate::FTResult<ApiResponse<T>> {
